...
> i **run fault di sviluppo (§6.2)**, verificati e conservati, in [§4.3](#run-fault-62); il **perimetro del codice Q8**, chiuso, in [§4.4](#perimetro-codice-q8); le **evidence 697-D**, verificate e conservate nella release v2, in [§4.5](#evidence-697-d). La Fase 03 resta aperta. La parte restante delle fasi successive
...
([§4.5](#evidence-697-d)). Il punto sul
...
La selezione del catalogo, i nuovi run fault e le relative evidence sono ora documentati in
[§4.1–§4.5](#412--fasi-successive). Restano `cal_thr` e `far_ver`, la produzione dei prototipi,
le decisioni di catalogo successive, il capability pilot e tutte le inferenze LLM. Prima di
dichiarare definitivo il congelamento della fase 02 resta la riverifica
...
alternativo, sonda sui prompt reali e gate 40×3; i nuovi run e le loro evidence sono ora
documentati in [§4.3](#run-fault-62) e [§4.5](#evidence-697-d).
...
Restano D2, producer alternativo, gate reale 03.0 e le sotto-fasi successive; specifica, nuovi run
ed evidence sono ora documentati in [§4.3](#run-fault-62) e [§4.5](#evidence-697-d). OOD e D11 sono ora lavorabili: la
...
[§4.5](#evidence-697-d). Non esistono ancora insight o prototipi, né risultati di accuracy; nessuna chiamata a
...
[§4.5](#evidence-697-d). Restano le altre dipendenze del report §7: insight e prototipi delle
sotto-fasi successive; manifest scientifico autonomo dei veri
...
La sotto-fase 03.6 ha applicato la regola function-level alle funzioni effettivamente usate e ha
reso espliciti i parametri; la 03.9 dovrà fare lo stesso nel proprio perimetro. Resta all'autore la scelta se proteggere
...
### 4.5 · Fase 03 — sotto-fase 03.6 — evidence 697-D
...
**non è chiusa** e questo branch non è ancora integrato in `main`.
...
function-level in [`PROVENIENZA.md`](../studio2/PROVENIENZA.md) §9.
...
seguente contabilità è ricostruita dall'indice e dal manifest, non dalla formulazione ambigua del
verbale:
...
- le evidence `normal_dev` non fanno parte di questo pacchetto;
- il lotto è condizionato a R2 e alla baseline attuale: il decadimento di R2 o il passaggio a
  `baseline_fit_new` lo invalida;
- la sotto-fase è verificata, ma la Fase 03 resta aperta e il branch non è integrato in `main`.
...
zero extra e zero AppleDouble; i metadati riportano inoltre zero membri con header PAX e zero
metadati xattr. La
v1 resta pubblicata e immutata: v2 è soltanto un nuovo packaging AppleDouble-free degli stessi
...
- provenienza function-level e U3:
  [`PROVENIENZA.md`](../studio2/PROVENIENZA.md) §9;
...
| :---: | --- | --- | --- | --- |
| 4 | **Preparazione e capability pilot — Fase 03** | Cantieri §6.1–§6.12 e gate §7.1; §4.1–§4.5 documentano criteri, catalogo D1, run fault, perimetro del codice ed evidence 697-D | piano §§6–7.1 e artefatti delle sotto-fasi | aperta; sotto-fasi documentate 03.1–03.4 e 03.6 chiuse |
...
4. **§6.4** — Dati strutturati, testi neutrali e firme di sviluppo prodotti e verificati nella sotto-fase 03.6 descritta in [§4.5](#evidence-697-d); le evidence `normal_dev` restano escluse
