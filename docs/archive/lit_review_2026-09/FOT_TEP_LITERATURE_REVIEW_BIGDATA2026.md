> **Documento archiviato — non è più il riferimento attivo.**
> Istantanea del 2 settembre 2026, conservata integralmente. Precede il controllo A+, la
> mitigazione C06, le baseline numeriche C02B e i venticinque lavori di federazione esaminati
> l'11 settembre: la PARTE A (ricostruzione dell'esperimento) e la PARTE J (A come information
> floor) sono quindi **superate**, non solo datate.
>
> Il giudizio di letteratura ancora valido è stato trasferito nella sezione 14 di
> `docs/fot_walkthrough_conversazione_v2.md`, che è **l'unico luogo attivo della letteratura**:
> PARTE F nei «lavori più vicini» (§14.5), PARTE G e H nella «tenuta della novità» (§14.6),
> OUTPUT 8 nella «priorità bibliografica» (§14.7), OUTPUT 9 nel perimetro della ricerca (§14.4).
>
> **Resta solo qui**, e attende una destinazione nel blueprint del paper: OUTPUT 3 (related-work
> taxonomy), OUTPUT 4 (claim audit), OUTPUT 6 (reviewer red team), OUTPUT 7 (cosa deve stare nel
> paper), OUTPUT 10 (positioning finale), PARTE E (matrice comparativa) e PARTE K (baseline che i
> reviewer potrebbero chiedere).
