"""Fail-closed runner for the 72-call B_LOCAL_FIRST_V1 screening."""
...
from .prompt_variant import PromptAssets, canonical_json, sha256_bytes, sha256_file
...
def verify_input_freeze() -> None:
    manifest = load_json(C06_ROOT / "C06_INPUT_FREEZE_MANIFEST.json")
    if manifest["status"] != "FROZEN_BEFORE_INFERENCE":
        raise RuntimeError("C06 input manifest is not frozen")
    for item in manifest["experiment_artifacts"]:
        path = ROOT / item["path"]
        if not path.is_file() or path.is_symlink():
            raise RuntimeError(f"frozen C06 artifact missing: {item['path']}")
        if sha256_file(path) != item["sha256"]:
            raise RuntimeError(f"frozen C06 artifact hash mismatch: {item['path']}")


def preflight() -> tuple[list[dict], PromptAssets]:
    verify_input_freeze()
    schedule = load_json(C06_ROOT / "screening_schedule.json")
    if len(schedule) != 72:
        raise RuntimeError("screening schedule must contain exactly 72 repetitions")
    if Counter(item["repetition"] for item in schedule) != Counter({1: 24, 2: 24, 3: 24}):
        raise RuntimeError("screening repetition counts mismatch")
    if any(item["condition"] != VARIANT for item in schedule):
        raise RuntimeError("unexpected screening condition")
    if [item["sequence_index"] for item in schedule] != list(range(72)):
        raise RuntimeError("screening sequence indices must be exactly 0..71")
    prompt_manifest = load_json(C06_ROOT / "prompt_hash_manifest.json")
    expected = {
        (item["agent_id"], item["physical_case_id"]): item
        for item in prompt_manifest["prompts"]
    }
    if len(expected) != 24 or prompt_manifest["prompt_count"] != 24:
        raise RuntimeError("prompt manifest must bind exactly 24 unique prompts")
    scheduled_keys = {
...
def main() -> None:
    ...
    parser.add_argument("--execute-screening", action="store_true")
    args = parser.parse_args()
    if args.preflight == args.execute_screening:
        parser.error("choose exactly one of --preflight or --execute-screening")
    ...
    if openai.__version__ != SDK_VERSION:
        raise RuntimeError(
            f"OpenAI SDK {SDK_VERSION} is required; found {openai.__version__}; no inference was started"
        )
    ...
    with ExclusiveLock(lock):
        ...
        for entry in schedule:
            ...
            print(
                f"{len(records):02d}/72 {entry['agent_id']} {entry['physical_case_id']} "
                f"R{entry['repetition']} -> {record['parsed_final_output']['predicted_label']}"
            )
        ...
        metadata = {
            "variant": VARIANT,
            "status": "SCREENING_INFERENCE_COMPLETE",
            "completed_at": utc_now(),
            "planned_repetition_records": 72,
