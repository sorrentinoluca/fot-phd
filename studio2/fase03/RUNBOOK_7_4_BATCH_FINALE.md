...
| Trasporto non osservato (7.4-FIX-RETRY-RETE) | `transport failure on <slot> (attempt n); unobserved: no byte received` | **Nessuna azione.** Connessione caduta/rifiutata, 5xx o 429, senza alcun byte di risposta: il runner ritenta lo slot nello stesso tratto fino a 3 volte (attese 30/60/120 s, quota retry). Al terzo retry fallito lo slot è un fallimento tecnico definitivo (`ABANDONED`, evento `note:unobserved_transport_abandoned:<id>`, `abandoned_unobserved_transport` nel riepilogo): dato mancante, riportato a parte, e il tratto prosegue. Un `FAILED` di questo tipo lasciato da un tratto interrotto si ritenta con `--resume`. Timeout, 401/403, altri 4xx restano nella riga seguente. Lo STOP a cinque fallimenti consecutivi resta. |
