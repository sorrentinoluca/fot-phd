...
def prepare(input_manifest_path: Path, snapshot: Path, output_dir: Path) -> dict[str, Any]:
    config = load_json(PREFLIGHT_CONFIG_PATH)
    manifest = load_json(input_manifest_path)
    token_count = offline_token_counter(snapshot)
    prompts = build_pilot_sample(manifest, config, token_count=token_count)
    ...
    plan = {
        ...
        "scope": "PRE_GATE_ONLY_NOT_STABILITY",
        "prepared_at": utc_now(),
