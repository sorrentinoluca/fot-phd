...
freeze diventa efficace soltanto dopo chiusura di 7.2-R, verifica indipendente di questo candidato,
commit finale accettato e tag annotato dell'autore.
...
### 3.2 Producer e dipendenza 7.2-R

Il protocollo congela **un solo template producer per entrambi i producer**: i byte del template
di remediation del pilot, SHA-256
`4306c5da6f0ebefcbce75d26d585caf82cc05ff85d5e7e6639e3f65b2d66de12`, comprensivo della
riga sulle forme canoniche degli identificatori e sulla corrispondenza con `variable_ids`.
L'hash è autenticato da
`harness/EVIDENZA_REMEDIATION_PRODUCER_122B_PILOT_03_03_13.manifest.json`, SHA file
`84165178b42f306f1fc6434e9ffc97f937e267643d9332bea2a3b4d4ad5f935c`.

La libreria 27B del pilot fu prodotta col template originale
`e7e80d59b2def37bea8f20186fc297583e2c8b0dea390a3351c7843d841619e7`; non può quindi chiudere
S19 per il batch finale. **7.2-R deve accettare due librerie complete generate o riusate contro il
template remediation identico; almeno la 27B deve essere rigenerata.** I riferimenti durevoli a
`G_P` e `G_A`, i loro SHA file/canonici e il verbale 7.2-R sono segnaposto bloccanti. Questa
sezione non anticipa l'esito di 7.2-PREP e non modifica `librerie/`.
...
| Producer-swap | `SWAP4`: quattro fault in-catalogo ancora da nominare; 8 run × 7 non proprietari; B-LF × 3 | libreria completa `G_A` | 672 |
...
Le fonti congelate fissano la cardinalità `4`, ma non identificano i quattro fault di `SWAP4`.
La scelta è quindi una decisione bloccante da prendere prima del freeze; non può essere dedotta
silenziosamente dai quattro fault di continuità.
...
| Libreria principale | `G_P=8` massimo finché 7.2-R non prova il riuso | 8 | 122B producer |
| Libreria alternativa | `G_A=8` massimo; almeno 27B da rigenerare | 8 | 27B producer |
...
| **Totale futuro massimo** | somma | **6.918** | **6.910 122B + 8 27B** |

Parametri congelati: `S=0`, `U=0`, `d=7`, `X=100`, `Q=0`; `G_P=8` e `G_A=8` sono massimi
conservativi. Dopo 7.2-R un riuso dimostrato può soltanto ridurre il numero effettivo, senza
trasferire la quota residua ad altre chiamate.

Il totale 7.174 del `REPORT_PILOT_03_13.md` differisce di **−256**: il report conservava gli
accantonamenti prudenziali E5 `192+64`, mentre questo protocollo esclude E5 perché non congelata.
Tutte le altre voci coincidono, compresi `d=7`, `X=100`, `Q=0` e i massimi `G_P/G_A=8/8`.

Il vecchio tetto storico di 3.700 è superato di **3.218 chiamate (+87,0%)**. È accettato perché
...
- media: `(6910×26,209 + 8×34,043)/3600 = 50,382 h`; con margine 20%:
  **60,459 h < 168 h**;
- p95: `(6910×36,661 + 8×37,167)/3600 = 70,451 h`; con margine 20%:
  **84,542 h < 168 h**.

Il p95 27B `37,167 s` è quello delle otto leaf finali valide del pilot. T5 è **PASS nel massimo
conservativo** sia alla media sia al p95. Restano circa 107,5 h di margine alla media e 83,5 h al
...
1. chiusura 7.2-R con `G_P` e `G_A`, relativi SHA e verifica S5/S18/S19;
2. sostituzione documentale dei soli segnaposto di §3.2 e §8, nuova verifica indipendente e tag
   dell'autore; una modifica a contenuto congelato è una nuova revisione, mai in place;
3. target finale fresco e ledger durevole con inventario, identità, quota e schedule autenticati;
4. canary iniziale PASS;
5. esecuzione delle tre passate scientifiche e dei canary giornalieri;
6. chiusura del ledger, verifica dei conteggi e solo dopo analisi.
...
- Qualunque superamento della quota per stage o del totale 6.918, schedule non autenticata,
  dipendenza S5/S18/S19 non chiusa, canary identity STOP, secondo giorno marcato, ledger
...
| T5 | PASS-C | §5: 60,459 h media e 84,542 h p95, entrambe <168 h | Sì fino al freeze |
...
| S5 | PENDING 7.2-R | segnaposto `G_P/G_A`; §3.2 | **Sì** |
...
| S8 | PENDING | `protocol.py` e template producer hanno SHA completi in §§2–3; casi `SWAP4` non nominati | **Sì** |
...
| S18 | PENDING artefatto finale | contratto one-field in renderer, ma diff `G_P` B↔E da allegare dopo S5 | **Sì** |
| S19 | PENDING 7.2-R | template unico congelato; parità su librerie finali non ancora verificata | **Sì** |
...
**Verdetto del candidato:** `NO-GO` alla materializzazione finché `SWAP4` non è deciso, S5/S18/S19
non sono chiusi da 7.2-R e il candidato aggiornato non riceve review/tag.
...
4. La finestra organizzativa effettiva è `W=7 giorni`, con accettazione
   `1,20×T<=168 h`.
5. Dopo il primo ciclo producer 122B, una sola remediation autorizzata ha aggiunto al template la
   regola sulle forme canoniche degli identificatori; la ripetizione completa 8/8 è la sorgente
   ammissibile. Il template remediation è quello unico del batch finale.
6. Il massimo futuro 6.918 supera il tetto storico 3.700 di 3.218 chiamate (+87,0%); il tetto era
...
8. E5 non entra nel protocollo finale perché approvata ma non congelata; i suoi 256 slot
   prudenziali non sono chiamate autorizzate.

## 10. Domande aperte e chiusura del freeze

1. **Quali quattro fault usare nel producer-swap?** Opzioni: (A) `F1/F8/F10/F13`, i quattro
   fault di continuità (**raccomandata**: gruppo strutturale già nominato, nessun nuovo sorteggio);
   (B) `F2/F3/F14/F15`, i quattro nuovi; (C) sorteggio deterministico 2+2, con seed e artefatto
   congelati prima delle chiamate.

Dopo la risposta: sostituire `SWAP4` senza modificare cardinalità o conteggio. Azioni non
discrezionali ancora necessarie: chiudere 7.2-R; inserire path/SHA delle librerie finali e del
verbale; produrre il diff B↔E e la tabella S19; rieseguire il conteggio (che può solo diminuire
rispetto ai massimi `G_P/G_A`); ottenere review indipendente; creare, solo su mandato dell'autore,
il tag proposto. Fino ad allora questo documento resta un candidato e non autorizza
materializzazione o chiamate.
