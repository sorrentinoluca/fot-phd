...
E=Path(__file__).resolve().parent;C=E.parent/'candidate';H=C/'studio2/fase03/harness';O=Path('/Users/luker/fot-tep-riverifica-harness-a219bd4-01a0a1ec/evidence')
...
check('technical-manifest',H/'HARNESS_OFFLINE_CANDIDATE.json','483c7db5ad4a763a8f41d084538c7a2cce6b2a7d799a5ba11d57da852cee3828',20826)
...
acq=H/'non_ok_a219bd4_20260915';a=json.loads((acq/'ACQUISITION_INVENTORY.json').read_text())
for r in a['files']:
 check('acquisition-source',r['source'],r['sha256'],r['bytes'])
 if r['storage']=='copy':check('acquisition-copy',C/r['copy'],r['sha256'],r['bytes'])
repro=H/'d03_evidence';d=json.loads((repro/'REPRODUCTION_INVENTORY.json').read_text())
for r in d['files']:
...
protected=prior['protected_scope'];scope={p:g('diff','--name-only','e9b60c5db77edfd3c06a29857e6ba5f61ebe139a','HEAD','--',p).decode() for p in protected};assert all(not v for v in scope.values())
...
(E/'technical.diff').write_bytes(g('diff','efa9f6f94194985104047d831c0b087dc460532d','HEAD','--','studio2/fase03/harness/ledger.py','studio2/fase03/harness/test_d03_contract.py','studio2/fase03/harness/CONTRATTO_ESECUZIONE_E_RIPRESA.md'))
(E/'technical_paths.txt').write_bytes(g('diff','--name-status','efa9f6f94194985104047d831c0b087dc460532d','HEAD'))

# Independent exact-blob test-first and protected-files checks.
first='567881abf06572812c00ccc0ed817d169b68fee6'
tf=json.loads((H/'d03_evidence/TEST_FIRST.json').read_text())
...
assert g('show',first+':studio2/fase03/harness/ledger.py')==g('show','a219bd469bbd280f56b7fa9cb56cda115b0975ed:studio2/fase03/harness/ledger.py')
assert sha(g('show',first+':studio2/fase03/harness/ledger.py'))==tf['runtime_sha256']
assert g('rev-parse','HEAD^').decode().strip()==first
fp=json.loads((H/'d03_evidence/runs/FINAL_TEST_PROVENANCE.json').read_text());check('final-test-provenance',C/fp['test_path'],fp['final_test_sha256'])
(E/'test_first_final.diff').write_bytes(g('diff',first,'HEAD','--',fp['test_path']))
protected_files=json.loads((H/'d03_evidence/runs/scope.json').read_text())['protected_files_unchanged']
assert len(protected_files)==176
for r in protected_files:
 check('protected-file',C/r['path'],r['sha256'],r['bytes']);assert (C/r['path']).read_bytes()==g('show','e9b60c5db77edfd3c06a29857e6ba5f61ebe139a:'+r['path'])
...
for name,sources in [('w_original',{'retry_proof_probes.py':O/'retry_proof_probes.py'}),('z_original',{'chain_probes.py':O/'z_original/evidence/chain_probes.py'}),('y_original',{'edge_probes.py':O/'y_original/evidence/edge_probes.py'}),('literal',{'extended_probes.py':O/'literal/evidence/extended_probes.py','additional_edges.py':O/'literal/evidence/additional_edges.py'}),('x23_adapted',{'extended_probes.py':O/'x23_adapted/evidence/extended_probes.py','additional_edges.py':O/'x23_adapted/evidence/additional_edges.py'})]:
