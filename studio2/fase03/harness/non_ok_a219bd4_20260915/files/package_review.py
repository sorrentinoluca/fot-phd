...
E=Path(__file__).resolve().parent;C=E.parent/'candidate';O=Path('/Users/luker/fot-tep-riverifica-harness-edb37f3-01a0a1ec/evidence')
...
assert current['identity']=='a219bd469bbd280f56b7fa9cb56cda115b0975ed\n3fb8e50c189b85b447503b8a1ac99c1741904a5b\n';assert current['status']=='## HEAD (no branch)\n'
...
tests={}
for name,n in [('targeted',111),('discovery',146)]:
...
scripts={}
for rel in ['z_original/evidence/chain_probes.py','y_original/evidence/edge_probes.py','literal/evidence/extended_probes.py','literal/evidence/additional_edges.py','x23_adapted/evidence/extended_probes.py','x23_adapted/evidence/additional_edges.py']:
 prior=O/'chain_probes.py' if rel.startswith('z_original') else O/rel
 assert sha(E/rel)==sha(prior);scripts[rel]=sha(E/rel)
(E/'RESULTS.json').write_text(json.dumps(dict(candidate='a219bd469bbd280f56b7fa9cb56cda115b0975ed',tree='3fb8e50c189b85b447503b8a1ac99c1741904a5b',verdict='NON OK',original_D01='CLOSED for documented original reproductions',original_D02='CLOSED for documented original reproductions',residual='D03: altered mandatory zero-token reconciliation proof contents accepted by normative confirmation',tests=tests,scripts_sha256=scripts,originals_mapped=50,no_tests_sum=True),indent=2)+'\n')
links=[]
for p in [E/'VERIFICA_D02.md',E/'MATRICE_50_METODI.md',E/'MATRICE_ESTENSIONI.md',E/'COMANDI.md']:
...
print(json.dumps(dict(verdict='NON OK',report_sha256=sha(E/'VERIFICA_D02.md'),manifest_sha256=sha(E/'SHA256SUMS'),regular_files=len(files)-1,bytes=sum(r.get('bytes',0) for r in files),symlinks=len(symlinks),preserved_repositories=len(repos),candidate_status=current['status']),indent=2))
