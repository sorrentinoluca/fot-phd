...
root=Path('/Users/luker/fot-tep-harness-0310-d04');out=Path(__file__).resolve().parent;base='97868f9d6ef281c2dd4ab1c6ffb67e2477ee5715';acquisition='bf7774f2d153ecc50f27ba095f77b612933b4d26'
git=lambda *args:subprocess.check_output(['git','-C',str(root),*args],text=True)
changed=git('diff','--name-only',acquisition).splitlines()
allowed={'studio2/fase03/harness/ledger.py','studio2/fase03/harness/CONTRATTO_ESECUZIONE_E_RIPRESA.md','studio2/fase03/harness/HARNESS_OFFLINE_CANDIDATE.json','studio2/fase03/harness/test_d03_contract.py'}
assert set(changed)<=allowed,changed
paths=['studio2/fase03/harness/'+n for n in ['metric_adapter.py','metrics.py','test_metric_raccordo.py','runtime.py','gate_rules.py','test_c01_c03.py','test_d01_replay.py','test_d02_predecessors.py','test_d03_contract.py']]+['studio2/fase03/producer_probe.py','studio2/fase03/run_pilot.py']
...
(out/'runtime.json').write_text(json.dumps(dict(utc=datetime.now(timezone.utc).isoformat(),python=sys.version,sqlite=sqlite3.sqlite_version,thread_id='01a0a204-abda-7a00-8466-f52f5bc84812',role='implementation, not independent review',rollout=str(rollout),last_turn_context=turns[-1],prior_reviewer='01a0a1ec-35a4-7870-9c39-9bf922d36c85 / gpt-6-astra / high',model_diversity_in_this_implementation=False,prior_claude_review='user-supplied Claude family review limited to D03; runtime family declared by reviewer'),indent=2)+'\n')
