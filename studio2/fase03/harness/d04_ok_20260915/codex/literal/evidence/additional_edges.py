...
class Edges(m.Extended):
    ...
    def test_X23_failed_gate_resume_and_zero_token_do_not_erase_failure(self):
        ...
        with patch.object(m.rp,'Provider',Fail),self.assertRaises(m.HarnessError):m.rp.run_stability_stage(t.prepared,t.results,ledger=t.ledger)
        with self.assertRaises(m.HarnessError):m.rp.run_stability_stage(t.prepared,t.results,ledger=t.ledger,resume=True)
        ...
        with self.assertRaises(m.HarnessError):m.rp.run_stability_stage(t.prepared,t.results,ledger=t.ledger,resume=True)
        self.assertEqual(len(t.consumer_calls),4);self.assertEqual(t.ledger.snapshot()['requests_cumulative'],12)
        self.assertIsNone(t.ledger.event('outcome:stability_gate'));self.note(snapshot=t.ledger.snapshot(),raw_count=len(t.ledger.stage_records('stability_gate')),result='incomplete; no retry or T3/T6 evaluation')
...
names=[n for n in Edges.__dict__ if n.startswith('test_X')]
