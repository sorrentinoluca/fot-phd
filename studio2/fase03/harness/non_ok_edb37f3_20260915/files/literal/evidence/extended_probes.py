...
"""
        procs=[subprocess.Popen([sys.executable,'-c',code,str(self.home),f'writer-{i}'],cwd=C,env=dict(os.environ,PYTHONPATH=str(C))) for i in range(12)]
...
"""
        ...
        def at_commit(c,event,sha,detail):
            if event=='outcome:producer_conformity':
                p=subprocess.Popen([sys.executable,'-c',code,str(self.home)],cwd=C,env=dict(os.environ,PYTHONPATH=str(C)));procs.append(p)
                import time
