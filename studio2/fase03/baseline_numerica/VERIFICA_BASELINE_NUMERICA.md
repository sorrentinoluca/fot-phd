...
**Commit verificato:** `fc927a663395fd88edecf69dc3f594c47c7b663e`  
**Branch osservato:** `codex/studio2-baseline-numerica`  
**Data:** 2026-09-14

Questa verifica è stata svolta in sola lettura sul candidato e sugli artefatti ignorati locali.
L'unica modifica prodotta dalla sessione è questo verbale. Non sono stati eseguiti commit, push,
merge, tag, release, simulazioni o valutazioni su dati test.

## Riscontri

| Esito | Punto | Fonte primaria e riscontro |
| :---: | --- | --- |
| ✅ | Identità e perimetro del candidato | `git rev-parse fc927a6` restituisce il commit completo sopra. Il diff da `46c0b62` tocca solo `studio2/PROVENIENZA.md` e nuovi/modificati sotto `studio2/fase03/baseline_numerica/`; nessun artefatto congelato di `phase_b/`, `icl/`, `ablation/`, `tep_*_v2/`, `reproducibility/` o `code/` è modificato. `BASELINE_FREEZE.json` è preservato byte per byte, SHA-256 `8ac1f1e72b23484395c2955c7d14ac333e61f8743d662dd92b933e37f90547c9`. |
...
| ❌ | Provenienza dell'autorizzazione | `studio2/PROVENIENZA.md` §10 afferma due volte che l'autore, con «3.9 - OK», avrebbe accettato anche la mancata raggiungibilità del piano rev. 7 da `origin/main`. Il testo primario dell'autore autorizza invece soltanto il riuso del pathname, la sufficienza limitata della tracciabilità e lo svolgimento del controllo sui warning; non autorizza la deroga al prerequisito `origin/main`. Inoltre oggi `origin/main` contiene `a572d1c`, mentre l'audit non attesta lo stato del ref remoto al momento del lancio. La formulazione estende quindi l'autorizzazione oltre la fonte e va corretta; l'eventuale stato al lancio deve restare non provato o essere sostenuto da una fonte primaria. |
| ❌ | Nome della fonte leakage | `studio2/PROVENIENZA.md` §10 cita `check_evidence_leakage.py`, che non esiste nella worktree evidence. Il file verificato e realmente caricato è `studio2/fase03/evidence/leakage.py`, con SHA-256 `c77ae5b11186c5b0df87b2f1df8800cb45fb25317e248fe8484d3e8283073887`. L'hash è corretto, il pathname no. |
| ⚠️ | Storia dei tentativi parziali di estrazione | Report e provenienza affermano che due directory parziali fallite sono state eliminate. Non resta un log o un artefatto primario che permetta al revisore di distinguere questa storia da una nota di sessione. Il fatto non incide sui 320 output riusciti, verificati integralmente, ma va rimosso/qualificato oppure accompagnato da una traccia conservata se lo si vuole mantenere come fatto verificabile. |
...
Il candidato non può però ricevere `OK` finché `studio2/PROVENIENZA.md` attribuisce all'autore
un'autorizzazione che il messaggio «3.9 - OK» non contiene e cita un pathname inesistente. Va inoltre
rimossa o resa esplicitamente non verificabile la storia dei due tentativi parziali di estrazione.
Dopo queste correzioni documentali serve una nuova verifica del commit candidato. Anche dopo un
eventuale `OK`, pubblicazione/riscaricamento, mapping 03.10, integrazione in `origin/main` e tag
restano pending: il lotto è accettato, la baseline è verificata, ma la sotto-fase non è ancora
integrata né congelata efficacemente.

## Comandi principali rieseguiti

- `git rev-parse fc927a6`; diff del perimetro e controlli di raggiungibilità/tag;
- verifica indipendente completa dei 40 `.xlsx` con `openpyxl` in read-only;
- MATLAB R2025b `-maca64 -batch` con solo `load_system`/`get_param`/`close_system(...,0)`;
- ricalcolo `verify_prototypes.py` verso un file temporaneo e confronto byte per byte;
- verifica completa di 320 manifest evidence e 1.280 file per-unità;
- chiamata diretta al parser `_normal_examples` del worktree 03.10;
- verifica SHA-256 e contenuto di tutti i 1.336 membri del tar locale;
- suite unitari 03.9 e `python3 docs/test_explanation.py`.
...
Costo documentale indicativo: circa 5.000 righe di testo/codice mirate più lettura automatica
integrale di 40 workbook, 1.280 file evidence e 1.336 membri dell'archivio.
