...
j=json.loads((O/'integrity.json').read_text());d=json.loads((O/'documents.json').read_text());s=json.loads((O/'supplement.json').read_text());a=json.loads((O/'acquisizione.json').read_text())
source_rows='\n'.join(f"| `{e['path']}` | `{e['commit']}` |" for e in s['sources_not_ancestors'])
file_rows='\n'.join(f"| `{e['path'].split('/')[-1]}` | {e['bytes']} | `{e['sha256']}` |" for e in j['package'])
fail_rows='\n'.join('- `'+x.removeprefix('FAIL: ')+'`' for x in j['guardian']['failure_identifiers'])
report=f'''NON OK su A — due correzioni documentali circoscritte richieste. OK su B come sola consegna esterna identificata per impronta; B non sana i rilievi del candidato A.
...
| Nuova copia isolata del verificatore | `{R}` |
...
Il controllo finale alle **{s['time_utc']}** conferma lo stesso main; i branch remoti
...
Prima della review sostanziale, alle **{a['utc']}**, è stata acquisita senza normalizzazioni
...
| --- | --- | ---: | --- |
| Blob della consegna in A | `CONSEGNA_GIT_A.md` | {a['A']['bytes']} | `{a['A']['sha256']}` |
| **B, consegna esterna corrente** | **`CONSEGNA_ESTERNA_B.md`** | **{a['B']['bytes']}** | **`{a['B']['sha256']}`** |
...
'''
p=R/'studio2/fase03/paper_sections/VERIFICA_RACCORDO_DOCUMENTALE_0315.md';p.write_text(report)
print(str(p),len(p.read_bytes()),hashlib.sha256(p.read_bytes()).hexdigest())
