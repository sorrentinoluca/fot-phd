...
static void mdlCheckParameters(SimStruct *S){
  ...
  for (i=0; i < ssGetSFcnParamsCount(S); i++){
	if (!mxIsEmpty(ssGetSFcnParam(S,i))){
      ...
      if (nEls != Plen[i]){
        sprintf(msg,"Error in parameter %i:  Length = %zu"
                	".  Expecting length = %zu", i+1, nEls, Plen[i]);
 	    ssSetErrorStatus(S,msg);
	    return;
      } //if (nEls != Plen[i]){
      if (i == 1){
        doublereal stream_id = *mxGetPr(ssGetSFcnParam(S,i));
        if (!mxIsFinite(stream_id) || stream_id < 0. ||
            stream_id > 9007199254740991. || floor(stream_id) != stream_id){
          sprintf(msg,"Error in parameter 2: stream id must be an exact non-negative integer <= 2^53-1");
          ssSetErrorStatus(S,msg);
          return;
        }
      }
...
static void mdlInitializeSizes(SimStruct *S){
  ...
  if((MSFlag & 0x20) > 0){
    ssSetErrorStatus(S,"MSFlag bit 0x20 is forbidden by the Studio 2 Philox protocol");
    return;
  }
...
static void mdlTerminate(SimStruct *S){
  ...
  CounterValue = mxCreateNumericMatrix(1, 1, mxUINT64_CLASS, mxREAL);
  *((uint64_t *)mxGetData(CounterValue)) = (*ModelData).randsd_.draw_counter;
  if (mexPutVariable("base", "fot_draw_counter_end", CounterValue) != 0){
    mxDestroyArray(CounterValue);
    mexErrMsgTxt("Unable to export the final Philox draw counter");
  }
  mxDestroyArray(CounterValue);
...
/* L300: */
  ...
  /* Studio 2 stream identifier and draw counter. */
  if (rseed == NULL){
    (*tcModelData).randsd_.stream_id = UINT64_C(0);
  }else{
    (*tcModelData).randsd_.stream_id = (uint64_t)*rseed;
  } //if (rseed == NULL){
  (*tcModelData).randsd_.draw_counter = UINT64_C(0);
...
static doublereal tesub7_(void *ModelData, integer *i__){
  ...
  /*--------------------------- Function Body ---------------------------*/
  draw_index = (*tcModelData).randsd_.draw_counter++;
  ret_val = fot_philox_uniform(
      UINT64_C(0x464f545445503032),
      (*tcModelData).randsd_.stream_id,
      draw_index
  );
  if (*i__ < 0){
    ret_val = ret_val * 2.0 - 1.0;
  }
